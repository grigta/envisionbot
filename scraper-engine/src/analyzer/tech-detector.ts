import type { TechStackItem, TechCategory, TechPattern } from '../types.js';

// ============================================
// TECH DETECTION CONTEXT
// ============================================
 ...
 */
export function detectTechStack(context: TechDetectionContext): TechStackItem[] {
  const detected: TechStackItem[] = [];
  const foundTechs = new Set<string>();

  for (const tech of TECH_PATTERNS) {
    if (foundTechs.has(tech.name)) continue;

    let confidence = 0;
    let detectedBy: TechStackItem['detectedBy'] = 'pattern';
    ...
    for (const patternRule of tech.patterns) {
      ...
      if (matchResult.matched) {
        ...
        detectedBy = patternRule.type as TechStackItem['detectedBy'];
...
function matchPattern(rule: TechPattern['patterns'][0], context: TechDetectionContext): MatchResult {
  const result: MatchResult = { matched: false, confidence: 0 };
  ...
  switch (rule.type) {
    ...
    case 'html': {
      if (rule.pattern && rule.pattern.test(context.html)) {
        ...
        if (match?.[1]) result.version = match[1];
      }
      break;
    }
 ...
 */
export function groupByCategory(techStack: TechStackItem[]): Record<TechCategory, TechStackItem[]> {
  const grouped: Partial<Record<TechCategory, TechStackItem[]>> = {};
  ...
  return grouped as Record<TechCategory, TechStackItem[]>;
}
