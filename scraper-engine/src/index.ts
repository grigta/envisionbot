...
export async function analyzeCompetitor(
  ...
  const { analysis: positioning, tokensUsed: posTokens } = await positioningAnalyzer.analyze(
    domain,
    crawlResult.pages,
    techStack
  );
  ...
  const { analysis: swot, tokensUsed: swotTokens } = await swotGenerator.generate(
    domain,
    crawlResult.pages,
    techStack,
    seoAnalysis,
  ...
  const { report: recReport, tokensUsed: recTokens } = await recommendationsGenerator.generate(
    positioning,
    swot,
    seoAnalysis,
    techStack
  ...
  const categoryMap: Record<string, 'seo' | 'content' | 'ux' | 'marketing' | 'technical' | 'positioning'> = {
