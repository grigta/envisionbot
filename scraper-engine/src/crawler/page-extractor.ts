 ...
 */
export function extractSEOData($: cheerio.CheerioAPI): SEOData {
  return {
    // Basic meta
 ...
 */
export function extractHeadings($: cheerio.CheerioAPI): HeadingsData {
  ...
  for (let i = 1; i <= 6; i++) {
    $(`h${i}`).each((_, el) => {
      ...
      if (text) {
        headings[`h${i}` as keyof HeadingsData].push(text);
      }
    });
  }
 ...
 */
export function extractImages($: cheerio.CheerioAPI, baseUrl: string): ImageData[] {
 ...
 */
export function extractLinks($: cheerio.CheerioAPI, baseUrl: string): LinkData[] {
 ...
 */
export function extractFromHtml(html: string, url: string): PageExtractionResult {
 ...
 */
export async function extractFromPage(page: Page, url: string): Promise<PageExtractionResult> {
