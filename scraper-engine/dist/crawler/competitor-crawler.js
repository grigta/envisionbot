...
import { createProxyConfiguration, getRandomUserAgent, getStealthLaunchArgs, FINGERPRINT_EVASION_SCRIPT, getCommonHeaders, parseRobotsTxt, isUrlAllowed, RateLimiter, } from './anti-detection.js';
import { extractFromPage, createCrawledPage, normalizeUrl, shouldCrawlUrl, extractDomain, } from './page-extractor.js';
import { fetchSitemap, sortByPriority } from './sitemap-parser.js';
import { detectTechStack } from '../analyzer/tech-detector.js';
// ============================================
// COMPETITOR CRAWLER
// ============================================
export class CompetitorCrawler {
    ...
    onProgress;
    aborted = false;
    constructor(competitorId, config = {}) {
        this.competitorId = competitorId;
        this.config = { ...DEFAULT_CRAWL_CONFIG, ...config };
        this.job = this.createJob();
        this.rateLimiter = new RateLimiter(this.config.minDelay, this.config.maxDelay, this.config.maxRequestsPerMinute || 20);
    }
    createJob() {
        return {
            id: randomUUID(),
        ...
        try {
            ...
            if (this.config.useSitemap) {
                ...
                for (const sitemapUrl of sitemapUrls) {
                    if (urlsToCrawl.length >= this.config.maxPages)
                        break;
    ...
    async runCrawler(_baseUrl, _domain, initialUrls) {
        ...
        const crawler = new PlaywrightCrawler({
            ...
            requestHandler: async ({ page, request, enqueueLinks, log }) => {
                ...
                try {
                    ...
                    const crawledPage = createCrawledPage(randomUUID(), this.competitorId, url, depth, extraction, this.job.id, 200, responseTime);
    ...
    analyzeStructure() {
        ...
        const hasBlog = topLevelFolders.some(f => /^\/(blog|news|articles|posts)/i.test(f));
        const hasProductCatalog = topLevelFolders.some(f => /^\/(products?|catalog|shop|store)/i.test(f));
        const hasDocumentation = topLevelFolders.some(f => /^\/(docs?|documentation|help|guide)/i.test(f));
        // Build root node
        const rootNode = {
            path: '/',
            depth: 0,
 ...
 */
export async function crawlCompetitor(competitorId, domain, config, onProgress) {
...
//# sourceMappingURL=competitor-crawler.js.map