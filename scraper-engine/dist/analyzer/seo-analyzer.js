...
export function analyzeSEO(pages) {
    ...
    const score = Math.round((titleScore * WEIGHTS.title +
    ...
    generateInsights(pages, titleAnalysis, descriptionAnalysis, headingsAnalysis, imagesAnalysis, linksAnalysis, contentAnalysis, opportunities, strengths);
    return {
        score,
        issues,
        opportunities,
...
// TITLE ANALYSIS
// ============================================
function analyzeTitles(pages, issues) {
    const titles = pages.map((p) => p.seo.title).filter(Boolean);
...
function calculateTitleScore(analysis) {
    let score = 100;
...
// ============================================
function analyzeDescriptions(pages, issues) {
    const descriptions = pages.map((p) => p.seo.metaDescription).filter(Boolean);
...
function calculateDescriptionScore(analysis) {
    let score = 100;
...
// HEADINGS ANALYSIS
// ============================================
function analyzeHeadings(pages, issues) {
    let multipleH1 = 0;
    let missingH1 = 0;
...
function calculateHeadingsScore(analysis) {
    let score = 100;
    if (!analysis.hasH1)
        score -= 30;
...
// IMAGES ANALYSIS
// ============================================
function analyzeImages(pages, issues) {
    let totalImages = 0;
    let withoutAlt = 0;
    let withAlt = 0;
    let lazyLoaded = 0;
...
function calculateImagesScore(analysis) {
    return analysis.altPercentage;
}
// ============================================
// LINKS ANALYSIS
// ============================================
function analyzeLinks(pages, issues) {
    let totalLinks = 0;
    let internalLinks = 0;
    let externalLinks = 0;
    let nofollow = 0;
...
function calculateLinksScore(analysis) {
    let score = 100;
    if (analysis.internalLinks === 0)
        score -= 30;
    if (analysis.brokenLinks > 0)
        score -= analysis.brokenLinks * 5;
    return Math.max(0, Math.min(100, score));
}
// ============================================
// CONTENT ANALYSIS
// ============================================
function analyzeContent(pages, issues) {
...
function calculateContentScore(analysis) {
    ...
    if (analysis.averageWordCount < 300)
        score -= 30;
    else if (analysis.averageWordCount < 500)
        score -= 15;
...
// TECHNICAL ANALYSIS
// ============================================
function analyzeTechnical(pages, issues) {
    let score = 100;
...
// INSIGHTS GENERATION
// ============================================
function generateInsights(pages, titleAnalysis, descriptionAnalysis, headingsAnalysis, imagesAnalysis, linksAnalysis, contentAnalysis, opportunities, strengths) {
    // Strengths
 ...
 */
export function getIssueSeverityColor(type) {
    switch (type) {
        case 'error':
            return 'red';
        case 'warning':
