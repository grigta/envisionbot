...
let deepAnalysisJob = null;
let newsCrawlJob = null;
let universalCrawlerJob = null;
let taskExecutorJob = null;
export function startScheduler() {
    ...
    healthCheckJob = schedule.scheduleJob(healthCheckCron, async () => {
        ...
        try {
            ...
            if (report && report.findings.some((f) => f.severity === "critical" || f.severity === "error")) {
                await sendNotification(`🚨 *Health Check Alert*\n\n` +
    ...
    deepAnalysisJob = schedule.scheduleJob({ rule: deepAnalysisCron, tz: timezone }, async () => {
        console.log("📊 Running scheduled deep analysis...");
        try {
            const report = await runDeepAnalysis();
            if (report) {
                await sendNotification(`📊 *Daily Analysis Complete*\n\n` +
    ...
    if (newsCrawlEnabled) {
        ...
        newsCrawlJob = schedule.scheduleJob({ rule: newsCrawlCron, tz: timezone }, async () => {
            ...
            try {
                ...
                if (result.success) {
                    await sendNotification(`📰 *News Crawl Complete*\n\n` +
        ...
        universalCrawlerJob = schedule.scheduleJob(crawlerCron, async () => {
            ...
            try {
                ...
                if (results.length > 0) {
                    ...
                    if (failed > 0) {
                        await sendNotification(`🕷️ *Crawler Check*\n\n` +
