 ...
 */
export async function runClaudeCode(workDir, prompt, options) {
 ...
 */
export async function runClaudeCodeTask(workDir, task, context) {
 ...
 */
export async function generateProject(workDir, projectConfig) {
 ...
 */
export async function runClaudeCodeStreaming(workDir, prompt, onStep, options) {
 ...
 */
function parseStreamEvent(event, stepNum, currentToolId) {
 ...
 */
export async function runAgentTaskStreaming(prompt, onStep, options) {
 ...
 */
export async function planIdeaWithClaudeCode(idea) {
 ...
 */
export async function analyzeProjectCodebase(repoPath, project, onStep, previousPlan) {
 ...
 */
function extractPlanMarkdown(output) {
    ...
    if (headerMatch && headerMatch.index !== undefined) {
        ...
        const cutIndex = Math.min(secondHeader > 0 ? secondHeader : Infinity, secondHeaderEn > 0 ? secondHeaderEn : Infinity);
 ...
 */
function parseAnalysisFromMarkdown(markdown) {
    const implemented = [];
    const missing = [];
    const technicalDebt = [];
    const risks = [];
    ...
    for (const match of todoMatches) {
        ...
        if (lowerLine.includes("test") || lowerLine.includes("review") || lowerLine.includes("тест"))
            type = "review";
        else if (lowerLine.includes("doc") || lowerLine.includes("readme") || lowerLine.includes("документ"))
            type = "documentation";
        else if (lowerLine.includes("security") || lowerLine.includes("auth") || lowerLine.includes("безопасн") || lowerLine.includes("аутентиф"))
            type = "security";
        else if (lowerLine.includes("refactor") || lowerLine.includes("improve") || lowerLine.includes("рефакт") || lowerLine.includes("улучш"))
            type = "improvement";
        else if (lowerLine.includes("fix") || lowerLine.includes("bug") || lowerLine.includes("исправ") || lowerLine.includes("баг"))
            type = "maintenance";
