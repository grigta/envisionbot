...
import { createRepoManager } from "./repo-manager.service.js";
import { PlanRepository } from "../repositories/plan.repository.js";
import { ProjectRepository } from "../repositories/project.repository.js";
import { TaskRepository } from "../repositories/task.repository.js";
import { analyzeProjectCodebase } from "../tools/claude-code.js";
export class ProjectAnalyzerService {
    ...
    async analyzeProject(projectId) {
        ...
        try {
            ...
            const cloneResult = await this.repoManager.cloneOrUpdate(projectId, project.repo, (step, progress) => {
            ...
            const analysisResult = await analyzeProjectCodebase(cloneResult.localPath, { name: project.name, repo: project.repo, phase: project.phase }, this.onStep, previousPlan // Pass previous plan for context
            );
     ...
     */
    async savePlan(projectId, markdown, analysis) {
        const now = Date.now();
     ...
     */
    async syncTasksFromPlan(projectId, analysis) {
        ...
        for (const suggestedTask of analysis.suggestedTasks) {
            ...
            const isDuplicate = existingTasks.some((t) => t.title.toLowerCase() === suggestedTask.title.toLowerCase());
            if (isDuplicate) {
                continue;
            }
            const task = {
     ...
     */
    async updatePlanOnTaskComplete(taskId) {
        ...
        if (match) {
            updatedMarkdown = updatedMarkdown.replace(todoPattern, `- [x] ${match[1]} (completed: ${new Date().toISOString().split("T")[0]})`);
     ...
     */
    async updateStatus(projectId, status, progress, currentStep, error) {
 ...
 */
export function createProjectAnalyzer(deps, options) {
