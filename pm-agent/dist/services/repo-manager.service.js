...
export class RepoManagerService {
    ...
    async getRepoInfo(repo) {
        try {
            const result = await execa("gh", ["repo", "view", repo, "--json", "owner,name,isPrivate,defaultBranchRef"], { timeout: 30000 });
     ...
     */
    async cloneOrUpdate(projectId, repo, onProgress) {
        ...
        try {
            ...
            await execa("gh", ["repo", "clone", repo, repoPath, "--", "--depth", "1"], { timeout: 300000 } // 5 minutes timeout for clone
            );
    ...
    async getRepoStats(projectId) {
        ...
        try {
            ...
            const findResult = await execa("find", [repoPath, "-type", "f", "-not", "-path", "*/.git/*"], { timeout: 30000 });
            const files = findResult.stdout.split("\n").filter(Boolean).length;
            // Count directories
            const dirResult = await execa("find", [repoPath, "-type", "d", "-not", "-path", "*/.git/*"], { timeout: 30000 });
            const directories = dirResult.stdout.split("\n").filter(Boolean).length;
            // Detect languages by file extensions
            const extensionResult = await execa("find", [repoPath, "-type", "f", "-not", "-path", "*/.git/*", "-name", "*.*"], { timeout: 30000 });
...
//# sourceMappingURL=repo-manager.service.js.map