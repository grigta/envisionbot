...
`;
     ...
     */
    updatePlanWithCompletion(content, task, result) {
        ...
        if (content.includes(completedTasksMarker)) {
            ...
            return content.replace(completedTasksMarker, `${completedTasksMarker}\n${completionEntry}`);
        }
        else {
     ...
     */
    buildCompletionEntry(task, result) {
        const filesSection = result.filesModified && result.filesModified.length > 0
...
`;
     ...
     */
    async getPlanContent(projectId) {
        try {
...
//# sourceMappingURL=plan-tracker.service.js.map