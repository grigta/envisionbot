...
export async function startTelegramBot() {
    ...
    bot.command("status", async (ctx) => {
        ...
        await ctx.reply(`📊 *PM Agent Status*\n\n` +
            ...
            `📈 Last deep analysis: ${lastDeepAnalysis}`, { parse_mode: "Markdown" });
    });
    ...
    bot.command("health", async (ctx) => {
        ...
        try {
            ...
            if (report) {
                await ctx.reply(`✅ Health check complete!\n\nSummary: ${report.summary.slice(0, 500)}${report.summary.length > 500 ? "..." : ""}\n\nFindings: ${report.findings.length}\nTasks generated: ${report.generatedTasks.length}`);
            }
        }
        catch (error) {
    ...
    bot.command("analysis", async (ctx) => {
        ...
        try {
            ...
            if (report) {
                await ctx.reply(`✅ Deep analysis complete!\n\nSummary: ${report.summary.slice(0, 500)}${report.summary.length > 500 ? "..." : ""}\n\nFindings: ${report.findings.length}\nTasks generated: ${report.generatedTasks.length}`);
            }
        }
        catch (error) {
    ...
    bot.on("callback_query", async (ctx) => {
        ...
        if (action === "approve") {
            ...
            if (pendingAction) {
                ...
                await ctx.reply(`📋 *Action Details*\n\n` +
...
async function sendActionApprovalMessage(ctx, actionId) {
...
export async function notifyPendingAction(actionId) {
    ...
    if (!action)
        return;
    try {
        const message = await bot.telegram.sendMessage(ADMIN_CHAT_ID, `🤖 *New Action Pending*\n\n` +
            `Type: ${action.action.type}\n` +
            `Description: ${action.action.description}\n\n` +
            `Expires: ${new Date(action.expiresAt).toLocaleString()}`, {
            parse_mode: "Markdown",
