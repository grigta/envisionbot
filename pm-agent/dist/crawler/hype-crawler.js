 ...
 */
function detectSource(url, emoji) {
    if (emoji === "⭐" || url.includes("github.com"))
        return "GitHub";
    if (emoji === "🤗" || url.includes("huggingface.co"))
        return "HuggingFace";
    if (emoji === "®️" || url.includes("replicate.com"))
        return "Replicate";
    if (emoji === "👽" || url.includes("reddit.com"))
        return "Reddit";
 ...
 */
export async function crawlHypeList(limit = 25) {
    const items = [];
    const crawler = new PuppeteerCrawler({
        ...
        async requestHandler({ page, log }) {
            ...
            const extractedItems = await page.evaluate((maxItems) => {
                const results = [];
                ...
                for (let i = 0; i < listItems.length && results.length < maxItems; i++) {
                    ...
                    if (liText.includes("⭐"))
                        emoji = "⭐";
                    else if (liText.includes("🤗"))
                        emoji = "🤗";
                    else if (liText.includes("®️"))
                        emoji = "®️";
                    else if (liText.includes("👽"))
                        emoji = "👽";
