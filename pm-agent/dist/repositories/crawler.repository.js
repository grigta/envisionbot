...
export class CrawlerRepository extends BaseRepository {
    ...
    cachePrefix = "pm:crawler";
    cacheTTL = 300; // 5 minutes
    pubsubChannel = "pm:events:news"; // Reuse news channel
    constructor(deps) {
        super(deps);
    }
    // ==========================================
    ...
    `;
        this.db.prepare(sql).run(fullSource.id, fullSource.name, fullSource.url, fullSource.prompt || null, fullSource.schema ? JSON.stringify(fullSource.schema) : null, fullSource.requiresBrowser ? 1 : 0, fullSource.crawlIntervalHours, fullSource.isEnabled ? 1 : 0, fullSource.createdAt, fullSource.updatedAt);
    ...
    async updateSource(id, updates) {
      ...
      WHERE id = ?
    `;
        this.db.prepare(sql).run(updated.name, updated.url, updated.prompt || null, updated.schema ? JSON.stringify(updated.schema) : null, updated.requiresBrowser ? 1 : 0, updated.crawlIntervalHours, updated.isEnabled ? 1 : 0, updated.updatedAt, id);
        await this.invalidateCachePattern(`${this.cachePrefix}:*`);
     ...
     */
    async updateCrawlResult(id, result) {
        const sql = `
      UPDATE ${this.tableName}
      ...
      WHERE id = ?
    `;
        this.db.prepare(sql).run(Date.now(), result.status, result.itemCount || null, result.error || null, Date.now(), id);
    ...
    `;
        this.db.prepare(sql).run(item.id, item.sourceId, item.title, item.url, item.description || null, item.content || null, JSON.stringify(item.metadata), item.extractedAt, item.projectId || null, item.relevanceScore || null, item.isProcessed ? 1 : 0);
        return item;
    }
    /**
     * Bulk upsert items
     ...
     */
    async markItemProcessed(id, projectId, relevanceScore) {
        const sql = `
     ...
     */
    async deleteOldItems(olderThanDays) {
        const cutoff = Date.now() - olderThanDays * 24 * 60 * 60 * 1000;
        const stmt = this.db.prepare("DELETE FROM crawled_items WHERE extracted_at < ? AND is_processed = 0");
     ...
     */
    async getStats() {
        const sourcesStmt = this.db.prepare(`SELECT COUNT(*) as total, SUM(CASE WHEN is_enabled = 1 THEN 1 ELSE 0 END) as enabled
       FROM ${this.tableName}`);
        const sourcesRow = sourcesStmt.get();
        const itemsStmt = this.db.prepare(`SELECT COUNT(*) as total, SUM(CASE WHEN is_processed = 1 THEN 1 ELSE 0 END) as processed
       FROM crawled_items`);
        const itemsRow = itemsStmt.get();
        const lastCrawlStmt = this.db.prepare(`SELECT MAX(last_crawl_at) as last_crawl FROM ${this.tableName} WHERE last_crawl_at IS NOT NULL`);
        const lastCrawlRow = lastCrawlStmt.get();
        return {
