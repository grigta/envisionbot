...
import type { ChatSession } from "../repositories/chat.repository.js";
...
export class MigrationService {
  ...
  private migrateStateFile(result: MigrationResult): void {
    ...
    const state: StateStore = JSON.parse(data);
  ...
  private migrateProjectsFile(result: MigrationResult): void {
    ...
    const projectsConfig = JSON.parse(data) as { projects?: Project[] };
  ...
  private migrateChatHistory(result: MigrationResult): void {
    ...
    const chatStore: ChatHistoryStore = JSON.parse(data);
  ...
  private insertPendingAction(action: PendingAction): void {
    this.db
      .prepare(
  ...
  private insertMetric(metric: ProjectMetrics): void {
    this.db
      .prepare(
  ...
  private insertReport(report: AnalysisReport): void {
    this.db
      .prepare(
  ...
  private insertIdea(idea: Idea): void {
    this.db
      .prepare(
        `INSERT INTO ideas (
