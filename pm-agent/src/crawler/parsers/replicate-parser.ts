 ...
 */
export async function parseReplicate(url: string): Promise<NewsItemDetails> {
    ...
    async requestHandler({ page, log }) {
      ...
      const extracted = await page.evaluate(() => {
        ...
        result.tags = Array.from(tagEls)
          ...
          .filter((t): t is string => !!t && t.length < 50 && !t.includes("Run"))
          .slice(0, 10);
        ...
        if (schemaText.includes("image")) result.inputTypes = [...(result.inputTypes || []), "image"];
        if (schemaText.includes("text")) result.inputTypes = [...(result.inputTypes || []), "text"];
        if (schemaText.includes("audio")) result.inputTypes = [...(result.inputTypes || []), "audio"];
        if (schemaText.includes("video")) result.inputTypes = [...(result.inputTypes || []), "video"];
