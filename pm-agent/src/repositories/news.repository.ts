 ...
 */
function newsItemToParams(item: NewsItem): Record<string, unknown> {
  return {
    id: item.id,
    rank: item.rank,
    title: item.title,
...
export class NewsRepository extends BaseRepository<NewsItem> {
  ...
  protected readonly pubsubChannel = "pm:events:news" as PubSubChannel;
  ...
  async getAll(filter?: {
    ...
    const cacheKey = this.listCacheKey(
      filter ? `${filter.source || "all"}_${filter.isActive ?? "all"}_${filter.limit || "all"}` : "all"
    );
    ...
    const rows = stmt.all(...params) as NewsItemRow[];
  ...
  async getById(id: string): Promise<NewsItem | undefined> {
    ...
    const row = stmt.get(id) as NewsItemRow | undefined;
   ...
   */
  async getTop(limit: number = 25): Promise<NewsItem[]> {
   ...
   */
  async getByRank(rank: number): Promise<NewsItem | undefined> {
    const stmt = this.db.prepare(
      `SELECT * FROM ${this.tableName} WHERE rank = ? AND is_active = 1`
    );
    const row = stmt.get(rank) as NewsItemRow | undefined;
   ...
   */
  async upsert(item: NewsItem): Promise<NewsItem> {
   ...
   * Bulk upsert items
   */
  async bulkUpsert(items: NewsItem[]): Promise<{ inserted: number; updated: number }> {
    let inserted = 0;
    let updated = 0;
   ...
   */
  async markInactive(excludeIds: string[]): Promise<number> {
   ...
   */
  async updateAIAnalysis(id: string, analysis: AIApplicationAnalysis): Promise<void> {
    const sql = `
      UPDATE ${this.tableName}
      SET
        ai_summary = ?,
    ...
    stmt.run(
      ...
      JSON.stringify(analysis.applications),
      JSON.stringify(analysis.projectIdeas),
      JSON.stringify(analysis.targetAudience),
      JSON.stringify(analysis.integrations),
      analysis.analyzedAt,
      Date.now(),
      id
    );
   ...
   * Get crawl history
   */
  async getCrawlHistory(limit: number = 10): Promise<CrawlHistory[]> {
    const stmt = this.db.prepare(`
    ...
    return rows.map((row) => ({
      ...
      status: row.status as CrawlHistory["status"],
   ...
   * Get last crawl
   */
  async getLastCrawl(): Promise<CrawlHistory | undefined> {
   ...
   */
  async saveCrawlHistory(history: CrawlHistory): Promise<number> {
    if (history.id) {
      // Update existing
      const sql = `
      ...
      this.db.prepare(sql).run(
        ...
        history.status,
        history.itemsFound,
        history.itemsUpdated || null,
        history.itemsNew || null,
        history.errors ? JSON.stringify(history.errors) : null,
  ...
  async getStats(): Promise<NewsStats> {
    ...
    const analyzedStmt = this.db.prepare(
      `SELECT COUNT(*) as count FROM ${this.tableName} WHERE ai_analyzed_at IS NOT NULL`
    );
    ...
    const bySourceRows = bySourceStmt.all() as Array<{ source: string; count: number }>;

    const bySource: Record<NewsSource, number> = {
      GitHub: 0,
      HuggingFace: 0,
      Replicate: 0,
      Reddit: 0,
    ...
    for (const row of bySourceRows) {
      bySource[row.source as NewsSource] = row.count;
    }
