...
export class PlanRepository extends BaseRepository<ProjectPlan> {
  ...
  private rowToPlan(row: PlanRow): ProjectPlan {
    return {
      id: row.id,
  ...
  private statusRowToStatus(row: AnalysisStatusRow): AnalysisStatus {
    return {
      projectId: row.project_id,
      status: row.status,
   ...
   */
  async getByProjectId(projectId: string): Promise<ProjectPlan | undefined> {
    const cacheKey = this.entityCacheKey(projectId);
    const cached = await this.getFromCache<ProjectPlan>(cacheKey);
    if (cached) return cached;

    const stmt = this.db.prepare("SELECT * FROM project_plans WHERE project_id = ?");
    const row = stmt.get(projectId) as PlanRow | undefined;
   ...
   * Get plan by ID
   */
  async getById(id: string): Promise<ProjectPlan | undefined> {
   ...
   */
  async upsert(plan: ProjectPlan): Promise<ProjectPlan> {
    ...
    await this.invalidateCache(
      this.entityCacheKey(plan.projectId),
      this.listCacheKey()
    );
   ...
   */
  async updateMarkdown(projectId: string, markdown: string): Promise<ProjectPlan | undefined> {
    const stmt = this.db.prepare(`
   ...
   * Get all plans
   */
  async getAll(): Promise<ProjectPlan[]> {
  ...
  async getAnalysisStatus(projectId: string): Promise<AnalysisStatus | undefined> {
    ...
    const row = stmt.get(projectId) as AnalysisStatusRow | undefined;
   ...
   */
  async updateAnalysisStatus(status: AnalysisStatus): Promise<void> {
    const stmt = this.db.prepare(`
   ...
   */
  async resetAnalysisStatus(projectId: string): Promise<void> {
    const stmt = this.db.prepare(`
   ...
   */
  async getRunningAnalyses(): Promise<AnalysisStatus[]> {
    const stmt = this.db.prepare(`
      SELECT * FROM analysis_status
      WHERE status NOT IN ('idle', 'completed', 'failed')
    `);
    const rows = stmt.all() as AnalysisStatusRow[];
  ...
  private versionRowToVersion(row: PlanVersionRow): PlanVersion {
    return {
      id: row.id,
      planId: row.plan_id,
      version: row.version,
   ...
   */
  async archiveCurrentVersion(planId: string, changeSummary?: string): Promise<void> {
   ...
   */
  async getVersions(projectId: string): Promise<PlanVersion[]> {
    ...
    const rows = stmt.all(plan.id) as PlanVersionRow[];
   ...
   */
  async getVersion(projectId: string, version: number): Promise<PlanVersion | undefined> {
    const plan = await this.getByProjectId(projectId);
    ...
    const row = stmt.get(plan.id, version) as PlanVersionRow | undefined;
   ...
   */
  async getAllVersionsWithCurrent(projectId: string): Promise<PlanVersion[]> {
    const plan = await this.getByProjectId(projectId);
    if (!plan) return [];
