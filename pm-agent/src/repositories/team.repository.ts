...
import type { TeamMember, TeamMemberRole, ProjectTeamMember } from "../types.js";
...
export class TeamMemberRepository extends BaseRepository<TeamMember> {
  ...
  async getAll(): Promise<TeamMember[]> {
    const cacheKey = "pm:team_members:all";
  ...
  async getActive(): Promise<TeamMember[]> {
    const cacheKey = "pm:team_members:active";
    const cached = await this.getFromCache<TeamMember[]>(cacheKey);
    if (cached) return cached;

    const stmt = this.db.prepare("SELECT * FROM team_members WHERE is_active = 1 ORDER BY name ASC");
    const rows = stmt.all() as TeamMemberRow[];
    const members = rows.map((row) => this.rowToTeamMember(row));
  ...
  async getByGithubUsername(username: string): Promise<TeamMember | undefined> {
    const stmt = this.db.prepare("SELECT * FROM team_members WHERE github_username = ?");
    const row = stmt.get(username) as TeamMemberRow | undefined;

    if (row) {
      const member = this.rowToTeamMember(row);
      await this.setCache(this.entityCacheKey(member.id), member);
      return member;
    }
  ...
  async getByEmail(email: string): Promise<TeamMember | undefined> {
    const stmt = this.db.prepare("SELECT * FROM team_members WHERE email = ?");
  ...
  async getByProjectId(projectId: string): Promise<TeamMember[]> {
    const cacheKey = `pm:team_members:project:${projectId}`;
    const cached = await this.getFromCache<TeamMember[]>(cacheKey);
    if (cached) return cached;
  ...
  async upsert(member: TeamMember): Promise<TeamMember> {
    const stmt = this.db.prepare(`
      INSERT INTO team_members (
        id, name, email, github_username, telegram_username,
  ...
  async update(id: string, updates: Partial<TeamMember>): Promise<TeamMember | undefined> {
    ...
    return this.upsert(updatedMember);
  }
  ...
  async addToProject(projectId: string, memberId: string, role?: TeamMemberRole): Promise<void> {
    const stmt = this.db.prepare(`
    ...
    await this.invalidateCache(`pm:team_members:project:${projectId}`);
  ...
  async getProjectTeamMembers(projectId: string): Promise<ProjectTeamMember[]> {
    ...
    const rows = stmt.all(projectId) as ProjectTeamMemberRow[];
    return rows.map((row) => ({
      id: row.id,
