...
import type { ToolDefinition, ToolResult, IdeaPlan } from "../types.js";
...
export async function executeIdeaTool(
  toolName: string,
...
async function handleIdeaAnalyze(input: Record<string, unknown>): Promise<ToolResult> {
  const { ideaId } = input as { ideaId: string; title: string; description: string };
...
async function handleIdeaSavePlan(input: Record<string, unknown>): Promise<ToolResult> {
  const { ideaId, plan } = input as { ideaId: string; plan: IdeaPlan };
...
async function handleIdeaCreateRepo(input: Record<string, unknown>): Promise<ToolResult> {
  const { ideaId, repoName, description, isPrivate } = input as {
    ideaId: string;
    repoName: string;
    description?: string;
    isPrivate?: boolean;
...
async function handleIdeaGenerateCode(input: Record<string, unknown>): Promise<ToolResult> {
  const { ideaId, repoPath, prompt } = input as {
    ideaId: string;
    repoPath: string;
    prompt: string;
  };
