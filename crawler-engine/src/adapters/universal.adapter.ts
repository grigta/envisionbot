...
export class UniversalAIAdapter implements CrawlerAdapter {
  private fetcher: Fetcher;
  private cleaner: HTMLCleaner;
  private converter: MarkdownConverter;
  private filter: PruningFilter;
   ...
   */
  private initWorkerPool(): Piscina {
    if (this.workerPool) {
   ...
   */
  async crawl(url: string, config: ExtractionConfig): Promise<CrawledItem[]> {
    const stats: Partial<CrawlStats> = {};
    ...
    return this.mergeResults(results, url, stats as CrawlStats);
  }
  ...
  private async extractInParallel(
  ...
  ): Promise<Array<{ data: unknown; raw: string }>> {
    ...
    const results = await Promise.all(
      payloads.map(payload => pool.run(payload))
    );
   ...
   */
  async crawlRaw(url: string): Promise<{
    html: string;
    cleanedHtml: string;
    markdown: string;
  ...
  private mergeResults(
    results: Array<{ data: unknown; raw: string }>,
    url: string,
    stats: CrawlStats
  ): CrawledItem[] {
    ...
    for (const result of results) {
      ...
      // Handle object with items array
        ...
        const itemsData = (data as { items: unknown[] }).items;
   ...
   */
  private normalizeToCrawledItem(data: unknown, sourceUrl: string): CrawledItem | null {
  ...
  async close(): Promise<void> {
    ...
    if (this.useBrowser && 'close' in this.fetcher) {
      await (this.fetcher as BrowserFetcher).close();
    }
...
export function createUniversalAdapter(config: AdapterConfig): UniversalAIAdapter {
