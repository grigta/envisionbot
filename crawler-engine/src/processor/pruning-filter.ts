...
export class PruningFilter {
   ...
   */
  private analyzeBlock(content: string): ContentBlock {
   ...
   */
  private shouldKeepBlock(block: ContentBlock): boolean {
   ...
   */
  private calculateTextDensity(markdown: string): number {
   ...
   */
  private calculateLinkDensity(markdown: string): number {
    const plainText = this.extractPlainText(markdown);
...
export function createPruningFilter(options?: Partial<PruningOptions>): PruningFilter {
